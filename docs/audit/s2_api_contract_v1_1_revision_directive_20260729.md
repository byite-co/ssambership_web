...
- **근거 문서:** 계약서 v1.0 2건(웹·앱), 1차 검증 보고서, 재검토서, 재검토서 타당성 분석(본 세션), 오너 최종 보정 지시(2026-07-29). 계약서 v1.0 원문은 본 저장소에 없음 — 개정 세션은 오너 보관본을 열어 절 번호를 대조할 것.
...
### A-3. M11 컬럼 단위 REVOKE는 무효 — 테이블 단위 전면 회수로 교체

- **결함(확정·실측):** `anon`·`authenticated`가 `mentor_profiles`·`mentor_plans`에 **테이블 단위** INSERT/UPDATE/DELETE(및 TRUNCATE) 권한 보유. PostgreSQL은 테이블 단위 권한이 있으면 컬럼 단위 REVOKE가 실효 없음 → v1.0의 M11은 적용해도 XW-02가 닫히지 않음.
- **정정:** M11을 `REVOKE INSERT, UPDATE, DELETE ON public.mentor_profiles, public.mentor_plans FROM anon, authenticated`로 교체. 단, 아래 **선행 조건 3건**을 M11 적용 게이트로 명시:
  1. A-4의 정산계좌 전용 RPC 적용 + 웹 호출부(`lib/mentor/mentorPayoutAccountActions.ts`) 전환 완료
  2. `syncAfterSignUpWithSession`의 백업 upsert 경로 제거(v1.0에 기재된 항목 유지)
  3. 멘토 플랜 저장 경로가 전부 RPC 경유임을 실측 재확인
...
- **정정:** v1.1 F12는 재생 판정을 정본 위임이 아니라 **결제 시점 불변 동의 금액** 기준으로 자체 수행. 다음 검증을 계약에 명시:
  ...
  재생 시 원장 대조는 `intent_amount_cents` 기준(현재 plan 가격 아님). T-CONC-09 기대값을 "가격 변경 후 재생 = `ok:true, idempotent:true`, anomaly 기록 없음"으로 재작성.
...
- **정정 1 (duplicate 대조):** F11 재생 경로는 기존 원장 행의 `user_id`·`ref_type`·`reason`·`delta_cents`(및 `p_idempotency_key = p_order_ref`)를 전건 대조하고 불일치 시 `LEDGER_FIELD_MISMATCH`. 이는 정본 checkout이 `sub_debit` 원장에 쓰는 확립된 관행과 일치.
...
- **게이트 순서(변경 금지):** ① 보상 삭제 대체 RPC 제공 → ② 앱 전환 배포·보급 → ③ `community_posts` DELETE 권한·`cp_delete_own` 정책 회수. M8은 이 3단계 게이트로 재작성.
...
v1.0의 마이그레이션 절(M8·M9 계열)이 인용한 객체명 중 3건이 라이브와 불일치(1차 검증 보고서 지적, 재검토서가 M8 1건 재확인). **주의:** 계약서 v1.0 원문과 1차 보고서 원문은 본 저장소에 없어, 3건의 원문 인용 문자열은 개정 세션이 오너 보관본에서 대조해야 한다. 교체 기준이 되는 **라이브 실측 정본**은 다음과 같다 (2026-07-29 `pg_policies` 측정):
...
3. E절의 stale 참조 3건은 v1.0 원문 대조로 원문 인용 문자열을 확정해 교체할 것.
4. 자금 함수 3종(`confirm_subscription_checkout`·`record_cash_topup`·renewal)의 본문 분기 순서는 요약 전재가 아니라 **원문 기준 재기술** ([C4] 오측 재발 방지).
5. 게이트: A-1~A-9 반영 + B 공용 구현부 전건 명세 + F 동기화 완료 시 S2-1 REVISE 해제, S2-2 NO-GO 해제 심사 가능.
