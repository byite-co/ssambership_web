...
| 2026-07-26 | `179_p1_10_account_deletion_consent_column_canon.sql` · `180_p1_10_account_deletion_forfeit_key_canon.sql` (W5-c) | ssambership-staging | Supabase MCP execute_sql — 2파일 순차 적용(각 파일 단일 배치). 공격 테스트 T27~T33 은 별도 배치 `begin; … rollback;` | W5-c 편차 해소. **179** `consented_balance_cents` 정본화: NULL 행 backfill(실측 total 1 · NULL 0행 — 멱등 no-op) → `DEFAULT 0` + `NOT NULL` 승격, 잔액 0 경로가 NULL 을 방치하던 **유일한 insert 함수** `account_deletion_request_consented(uuid,int,boolean,boolean,bigint)` 재정의(`v_consented := 0` 명시 — request/request_self/request_self_consented 는 전부 이 함수로 위임하므로 재정의 1개로 전 경로 커버), 동의 "행위" 유무는 `forfeit_consent_at` 단독 판정으로 일원화, STALE·REQUIRED 가드의 `coalesce(v_consented,0)` 은 방어적 중복으로 존치. **180** 몰수 멱등키 정본 확정(`acct_del_forfeit:{uid}` / `account_deletion_forfeit`) + 레거시 `forfeit_on_deletion:{uid}` **이중 몰수 구조적 차단**: `account_deletion_forfeit_and_anonymize(uuid)` 재정의 — 잔액>0 분기의 원장 insert 전에 레거시 키 존재 시 `LEGACY_FORFEIT_LEDGER_PRESENT` 로 fail-closed(insert·지갑 차감·익명화 0, `record_error` 로 운영자 검토 대상 기록, current_balance_cents·legacy_idempotency_key 반환). staging 은 양 키 0행 실측이지만 production 원장 미실측 — 측정이 아니라 구조로 0 을 만든다. 원장 소급 수정 0(append-only). **rollback-only 공격 테스트 전건 PASS**: T27(잔액0 job → consented=0·NULL 아님·consent_at null) / T28(지갑 +100 후 begin_locked → **FORFEIT_CONSENT_REQUIRED**·pending 유지 — 지시서 명목은 STALE 이나 §3-1(c)가 잔액0 경로의 forfeit_consent_at 을 null 로 유지하므로 STALE 은 구조적으로 불가, 차단·pending 유지 계약은 동일 충족. 편차로 기록) / T29(is_nullable=NO·default=0) / T30(NULL 0행) / T31(레거시+잔액100·storage_purged → code=LEGACY_FORFEIT_LEDGER_PRESENT·정본 원장 0행·지갑 100 불변·익명화 0·attempts 1) / T32(레거시 없음 → 정본 키 정확히 1행·지갑 0·재호출 멱등 ok·원장 1행 유지 — T11·T12 재확인) / T33(레거시+잔액0 → ok=true·몰수 분기 미진입·익명화 진행). **owner_id 13버킷 전수 실측**(§2-1): community-post-images 64/64 · custom-order-deliverables 2/2 · custom-request-application-attachments 2/2 · custom-request-post-attachments 2/2 · profile-avatars 1/1 · question-room-attachments 7/7 · student-id-images 1/1 · individual-question-attachments 0/5(전부 owner NULL, DB 조인 커버) · connection-note-ink/custom-order-message-attachments/scan-annotations/shortform-thumbnails/shortform-videos 0/0 → **§3-3 분기 (b)**: TS 계층에서 수집 = DB 조인 ∪ owner_id(전 버킷) ∪ prefix 인벤토리, OWNERSHIP_CONFLICT/UNATTRIBUTABLE ≥1 → real-run 시작 0·storage_purged 전이 0(미커버 버킷과 동일 관문, 계약 테스트 12건 신설 — SQL 아님, 커밋 1168111). 종료 후 baseline 대조 **전 항목 일치**: fixture 잔여 0 · cash_ledger forfeit 양 키 0행 불변 · account_deletion_jobs 1행(NULL consent 0) · user_deletion_log 0. md5: 179 `17a79407f66f6894337164fb1d0f7702` · 180 `c00e6f41b88fa9f725130aa8e76fa95e`. ⚠ 전송 텍스트는 파일과 **실행 의미 동일**이나 파일 머리 주석 블록은 전송에서 요약됨(md5 는 저장소 파일 기준). ⚠ 2세션 동시성(C1~C3)·Auth API 실증은 env 부재로 미결(`ENV_MISSING_SUPABASE_DB_URL`·`ENV_MISSING_AUTH_KEYS`) — `CONCURRENCY_RUNTIME_NOT_VERIFIED` 존치. SQL 167~178 diff 0 · 172 결번 유지 · `ACCOUNT_DELETION_WORKER_ENABLED` 기본 false 불변 |
| 2026-07-26 | 179·180 **이중 적용 기록** (W5-e E1) | ssambership-staging | — (기록 전용 행 — 적용 없음) | 01c 가 두 세션에서 실행됨. A라인(`claude/w5-c-variance-concurrency-d12hk2` @ `7f25a2cbe1df6e02d60a4a0346372d38d38084d9`, 179 md5 `f7acfa4def7629de8e4079414c954b91` / 180 `8f71ca32593f424ec20bf7ddf852f402`)이 먼저 staging 적용, 이후 B라인(`claude/web-app-fixes-bug-rollback-cx52cq` @ `4804a7933811f26e05e6743bc9ffa6af4b46fcc3`, 179 md5 `17a79407f66f6894337164fb1d0f7702` / 180 `c00e6f41b88fa9f725130aa8e76fa95e`)이 재적용되어 **최종 live 함수 = B본**. 정본 = cx52cq. d12hk2 는 기록용 존치·병합 금지. B본 결함 2건(① `request_consented` 무잠금 동의 읽기 — live prosrc 'for update' 부재 실측, ② owner 증거 어댑터의 PostgREST storage 스키마 조회 — PGRST106 read-only 재현: `Invalid schema: storage`, 노출 스키마 public·graphql_public 뿐)은 **181·182 로 정정** — 아래 행 참조 |
| 2026-07-26 | `181_p1_10_account_deletion_storage_owner_refs.sql` (W5-e E2, md5 `1486cdcbf8797f827ca89d1403193261`) | ssambership-staging | Supabase MCP execute_sql 단일 배치 — 커밋 전문과 실행 의미 동일. 검증 T34~T36 은 별도 배치·mutation 0 | owner 증거 수집 PGRST106 해소. **사전 재현(read-only)**: anon PostgREST `Accept-Profile: storage` GET → `{"code":"PGRST106","message":"Invalid schema: storage"}` HTTP 406(노출 스키마 public·graphql_public 뿐). 신설 `account_deletion_storage_owner_refs(p_user_id uuid) returns table(bucket_id text, name text)` — SECURITY DEFINER·`search_path=public`·storage.objects 스키마 한정 참조·`owner_id = p_user_id::text`(owner_id **text** 타입 information_schema 실측 §2-1)·반환 2컬럼만(id·metadata·owner 미반환)·177/180 동일 패턴. **T34** PASS: anon·authenticated EXECUTE false · proacl `{postgres=X/postgres,service_role=X/postgres}`(PUBLIC `=X` 엔트리 없음) · secdef true · search_path=public / 런타임: `set role anon` 호출 42501 · PostgREST anon RPC 호출 42501(HTTP 401 — **PGRST106 아님**, 경로 구조 해소 입증). **T35** PASS: owner_id 보유 실사용자(61객체) uid 로 `set role service_role` 호출 → 61행 = 실측 61 · 버킷 집합 일치. **T36** PASS: 임의 신규 uuid → 0행 · `pg_get_function_result` = `TABLE(bucket_id text, name text)`. **어댑터 전환**: `makeResolveObjectOwners` 의 PostgREST storage 직조회 2건 → RPC 페이지네이션 호출 + 순수 매핑 `buildObjectOwnershipFromOwnerRefRows`(실패 시 예외 → fail-closed 현행 유지). owners 맵이 대상 uid 엔트리만 싣게 되는 의미 변화(객체별 타인-owner 값 관측 불가 → DB 조인 ref 의 타인-owner 이상치는 conflict 대신 삭제 대상으로 수렴)는 타입·워커 주석과 계약 테스트로 박제. 계약 테스트 5건 신설(261→266 전건 PASS). ⚠ **실 planner(service role 클라이언트) staging 1회 실행은 `ENV_MISSING: SUPABASE_SERVICE_ROLE_KEY` 로 미결** — RPC 실행(T35)·PostgREST 도달성(42501)·어댑터 형상(계약 테스트)으로 분해 입증, supabase-js `.rpc().range()` 실왕복만 UNVERIFIED |
...
| M17 | 미생성 | 미생성 | 미생성 | — | 미적용 | 미적용 | 미적용 | 미적용 |
| M8 | 미생성 | 미생성 | 미생성 | — | 미적용 | 미적용 | 미적용 | 미적용 |
| M14 | 미생성 | 미생성 | 미생성 | — | 미적용 | 미적용 | 미적용 | 미적용 |
| M9 | 미생성 | 미생성 | 미생성 | — | 미적용 | 미적용 | 미적용 | 미적용 |
...
| M9 | 미생성 | 미생성 | 미생성 | 미생성 | 웹 호출점을 레거시 함수로 선복원(§22 #3·#4), 레거시 구 본문 복원 포함 | 4 | 동일 |
| M14 | 미생성 | 미생성 | 미생성 | 미생성 | C11 호출부 선복원 | 5 | 동일 |
| M8 | 미생성 | 미생성 | 미생성 | 미생성 | C6 호출부 선복원 | 6 | 동일 |
| M17 | 미생성 | 미생성 | 미생성 | 미생성 | 앱 호출부 선복원 → `api_app_v1` 호출 0건 실측 → Exposed schemas 제거 → config 반영 확인 → DROP → schema cache reload(§22 M17 6단계). M7·M5 rollback보다 선행 | 7 | 동일 |
