...
| 2026-07-26 | `179_p1_10_account_deletion_consent_column_canon.sql` · `180_p1_10_account_deletion_forfeit_key_canon.sql` (W5-c) | ssambership-staging | Supabase MCP execute_sql — 2파일 순차 적용(각 파일 단일 배치). 공격 테스트는 별도 배치에서 `begin; … rollback;` | W5-c 편차 해소. **preflight 실측 4종**: ① `storage.objects.owner_id` 13버킷 전수 — with_owner>0 다수(community-post-images 64/64 · question-room-attachments 7/7 · custom-order-deliverables 2/2 · custom-request-application/post-attachments 각 2/2 · profile-avatars 1/1 · student-id-images 1/1), individual-question-attachments **0/5 전부 NULL**(등록 RPC 경유 — owner 미기록), 나머지 4버킷 빈 버킷 ② 몰수 멱등키 양 키 0행(`forfeit_on_deletion:%` 0 · `acct_del_forfeit:%` 0 — 직전 실측과 동일) ③ `account_deletion_jobs` 총 1행 · `consented_balance_cents` NULL 1행 ④ 충전 정본 RPC = `record_cash_topup(uuid,bigint,text)`(SQL 020, 저장소 유일 충전 함수 — T10 이 쓴 경로), 차단은 `adg_cash_wallets`/`adg_cash_ledger` 트리거의 `ACCOUNT_DELETION_IN_PROGRESS`. job insert 경로 전수(pg_proc 본문 실측): **`account_deletion_request_consented` 단 1함수**(request/request_self/request_self_consented 는 위임). **179** 동의 컬럼 정본화: NULL 1행 backfill=0 → `set default 0` + `set not null`, 유일 insert 경로 재정의(잔액 0 → `consented_balance_cents=0` 명시 기록·`forfeit_consent_at` null 유지, 잔액>0 동의 경로 잔액 읽기를 FOR UPDATE 로 정본화 — 176 실측은 일반 읽기), 위임 3종·STALE/REQUIRED 가드의 coalesce(v_consented,0) 불변. rollback-only 공격 테스트 **T27~T30 전건 PASS**(잔액0 job → 0 명시·NULL 아님 / 지갑+100 후 begin_locked fail-closed·pending 유지 / is_nullable=NO·default 0 / backfill 후 NULL 0행). ⚠ **T28 편차 실측**: 잔액0 job 은 (c) 계약상 forfeit_consent_at 이 null 이므로 begin_locked 는 **FORFEIT_CONSENT_REQUIRED** 를 먼저 반환(지시서 T28 기대 `FORFEIT_CONSENT_STALE` 은 (c)·(d) "가드 불변" 계약과 내부 모순 — 가드 순서 무수정, fail-closed·pending 유지·파괴 0 은 동일). STALE 가드 생존은 T28b(동의 100 기록 → 충전 200 → STALE·pending 유지)로 별도 증명. **180** 몰수 멱등키 정본 박제(`acct_del_forfeit:{uid}` / `account_deletion_forfeit`; 레거시 `forfeit_on_deletion:{uid}` 는 웹 즉시 경로 폐지로 소멸 — 원장 소급 수정 0) + `account_deletion_forfeit_and_anonymize` 재정의: 잔액>0 분기 **첫 검사**로 레거시 키 원장 행 존재 시 신규 원장 insert·지갑 차감·익명화 전부 중단, `ok=false, code=LEGACY_FORFEIT_LEDGER_PRESENT, current_balance_cents, legacy_idempotency_key` fail-closed 반환 + record_error(운영자 검토). staging 양 키 0행이지만 production 원장 미실측 — 측정 아닌 구조로 이중 몰수 0. **T31~T33 전건 PASS**(레거시+잔액100+storage_purged → 신규 원장 0·지갑 100 불변·익명화 미진행·attempts+1·last_error 기록 / 레거시 없음 → 정본 키 정확히 1행·지갑 0·재호출 멱등 1행 유지 / 레거시+잔액 0 → 가드 미발동·신규 원장 0·익명화 진행). **§3-3 owner_id (b) 분기**(with_owner>0 실측): `classifyDeletionPlan` 신설 — 수집 = DB 조인 refs ∪ owner_id=대상(전 버킷) ∪ prefix ∪ 유저-스코프 전 버킷 인벤토리, 4분류(정상 / DB O·owner 타인=OWNERSHIP_CONFLICT / DB X·owner=대상=owner 귀속 별도 표기 / 셋 다 아님=UNATTRIBUTABLE), owner 부재(null)는 충돌 아님(individual-question-attachments 실측 근거). 차단은 미커버 버킷과 **동일 관문**: 충돌·판별불능 ≥1 → real-run 시작 0(pending 게이트, locked 진입 전) · removeObjects 전 차단 · purging→storage_purged 전이 0 · record_error. 어댑터 `makeGatherOwnerEvidence` 는 storage 스키마 REST 조회 — ★실측: **storage 스키마 PostgREST 미노출(PGRST106**, exposed = public·graphql_public) → 현 구성에서 어댑터 예외 = 워커 fail-closed 정지(파괴 0). 해제는 Exposed schemas 에 storage 추가(코드 무변경) 또는 후속 회차 전용 RPC(이번 SQL 예산 179·180 뿐). Storage REST list/info 는 owner 미반환(대안 불가). 계약 테스트 신설 10건(분류표 스냅샷 · 충돌/판별불능 각 시작 0·전이 0 · 재검증 레이스 · dry-run 보고) — **N_start 249 → N_end 259 전건 PASS**, tsc 0 오류. 종료 후 baseline 대조 **전 항목 일치**: auth.users 8 · public.users 8 · account_deletion_jobs 1(canceled) · cash_ledger 6(forfeit 양 키 0) · user_deletion_log 0 · storage 84 · fixture 잔여 0. md5: 179 `f7acfa4def7629de8e4079414c954b91` · 180 `8f71ca32593f424ec20bf7ddf852f402`(⚠ 전송 텍스트는 실행 의미 동일 — 파일 머리 해설·§V·ROLLBACK 주석 블록은 전송 생략). ⚠ **G5 동시성 C1~C3 미실행: `ENV_MISSING_SUPABASE_DB_URL`**(직접 Postgres 접속 문자열 부재 — `CONCURRENCY_RUNTIME_NOT_VERIFIED` 존속, MCP 단일 호출로는 2세션 lock 대기 재현 불가·raw UPDATE 흉내 금지 준수) · **G6 Auth API 세션 폐기 미실행: `ENV_MISSING_AUTH_KEYS`**(`SUPABASE_SERVICE_ROLE_KEY`·anon/publishable key 부재). SQL 167~178 diff 0 · 172 결번 유지 · 151/154/161/175/176 파일 무수정 · `ACCOUNT_DELETION_WORKER_ENABLED` 기본 false 불변 · worker/cron 실행 0 · production 접속 0 |
