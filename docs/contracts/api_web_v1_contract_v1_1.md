...
| **M16(HD-1) 이전** | §14.7 확대 게이트 7단계 — F4/F5/F6 웹·앱 전환 + 멱등 재시도 구현 + 앱 보상 DELETE 제거 + 직접 쓰기 0건 확인 + service_role moderation 예외 목록화 |
...
2. **역순 적용**: M16 → M12 → M11 → **M10** → M9 → M8 → M7 → M6 → M5 → M4 → M14 → M13 → M15 → M1 → (**M0는 마지막이며 되도록 남긴다** — §20.5). (M2·M3는 retired — 롤백 대상 자체가 없다.)
...
승인·대조 통과 후 권고 착수 순서: **M0(필수·즉시) · M15(독립·조기) → M1 → M13 → M4 → C1 → M5 → M6 → C2·C3·C4 → M7 → C5 → M8 → C6 → M14 → C11 → M9 → C7·C8 → M10 → (호출 0건 확인) → M11 → M12 → (HD-1 게이트 7단계 충족 후) M16 → C9·C10** (M2·M3는 retired — 적용 대상 아님)
