...
//     Batch A 전건 < Batch B 전건 (물리 계획표 §9 생성 순서).
...
//     현재 정규 clean-install 175+5=180 (최종 목표 175+16=191 과 혼동 금지).
...
for (const batch of [S2_BATCH_A, S2_BATCH_B]) {
...
{
  const aMax = Math.max(...S2_BATCH_A.map((x) => byStem.get(x.stem)?.ts ?? "0").map(Number));
  const bMin = Math.min(...S2_BATCH_B.map((x) => byStem.get(x.stem)?.ts ?? "99999999999999").map(Number));
  if (Number.isFinite(aMax) && Number.isFinite(bMin) && !(aMax < bMin)) {
    fail(`Batch A 전건 timestamp < Batch B 전건 timestamp 위반 (A max=${aMax}, B min=${bMin})`);
  }
}
