-- =============================================================================
-- 20260804113000_iq_attachment_message_author_guard_rollback.sql
-- =============================================================================
-- forward(20260804113000)가 추가한 MESSAGE_AUTHOR_MISMATCH 가드를 제거하고
-- 함수 본문을 직전 배포 정본(20260803162808, prosrc md5
-- 58f0c2411d40b2ce3bcec23efa0c88a1)으로 문자 그대로 되돌린다.
-- ACL 은 forward 와 동일하게 무변경(CREATE OR REPLACE 는 기존 ACL 보존).
-- =============================================================================
begin;

-- 사전 게이트: forward 가 적용된 상태여야 한다.
do $$
declare
...
begin
  ...
  if position('MESSAGE_AUTHOR_MISMATCH' in v_src) = 0 then
    raise exception 'forward 20260804113000 not applied — nothing to roll back';
  end if;
end
$$;
...
-- 자가 검증: 가드 제거 + 기준선 본문(md5) 완전 일치 + ACL 불변.
do $$
declare
  v_oid oid := to_regprocedure('public.add_individual_question_attachment(uuid,text,text,text,uuid)')::oid;
  v_src text;
begin
  select prosrc into v_src from pg_proc where oid = v_oid;
  if position('MESSAGE_AUTHOR_MISMATCH' in v_src) > 0 then
    raise exception 'guard still present after rollback';
  end if;
  if md5(v_src) <> '58f0c2411d40b2ce3bcec23efa0c88a1' then
    raise exception 'rollback body differs from 20260803162808 baseline (md5 %)', md5(v_src);
  end if;
