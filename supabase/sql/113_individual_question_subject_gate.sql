-- 개별질문 공개 목록/맡기를 멘토 담당 과목(mentor_profiles.teaching_subjects)으로 제한.
-- 과목 없는(null) 질문은 일반 질문으로 모두에게 노출/허용.
...
begin
  ...
  return query
    select q.id, q.subject, q.topic, q.title, q.price_cents, q.expires_at, q.created_at
...
begin
  ...
  select q.required_school_tier, q.required_major_category, q.subject
    into v_required_school_tier, v_required_major_category, v_question_subject
  from public.individual_questions q
  where q.id = p_question_id and q.question_type = 'open' and q.status = 'open'
    and q.claimed_mentor_id is null and (q.expires_at is null or q.expires_at > now())
  ...
  -- 과목 게이트: 담당 과목이 아닌 질문은 거부(과목 없는 질문은 허용)
  if v_question_subject is not null then
    select mp.teaching_subjects into v_mentor_subjects from public.mentor_profiles mp where mp.user_id = p_mentor_id;
  ...
  if v_required_school_tier is not null or v_required_major_category is not null then
    select msv.school_tier, msv.verified_major_category into v_verified_school_tier, v_verified_major_category
    from public.mentor_school_verifications msv
    where msv.mentor_id = p_mentor_id and msv.status = 'approved'
    ...
    if v_required_school_tier is not null and coalesce(v_verified_school_tier, '') <> v_required_school_tier then
    ...
    if v_required_major_category is not null and coalesce(v_verified_major_category, '') <> v_required_major_category then
  ...
  set claimed_mentor_id = p_mentor_id, claimed_at = now(), status = 'claimed'
  where id = p_question_id and question_type = 'open' and status = 'open'
    and claimed_mentor_id is null and (expires_at is null or expires_at > now())
